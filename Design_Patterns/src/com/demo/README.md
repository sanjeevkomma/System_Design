...
| :---| :--- | :--- |:--- |:--- | 
|1 | [Factory Pattern(Simplified version of Factory Method)](https://www.oodesign.com/factory-pattern)| creates objects without exposing the instantiation logic to the client and refers to the newly created object through a common interface  | a framework delegate the creation of objects derived from a common superclass to the factory - we need flexibility in adding new types of objects that must be created by the class | 1. factories providing an xml parser: javax.xml.parsers.DocumentBuilderFactory or javax.xml.parsers.SAXParserFactory <br>2. java.net.URLConnection - allows users to decide which protocol to use |
|2 | Factory Method Pattern | Defines an interface for creating objects, but let subclasses to decide which class to instantiate and Refers to the newly created object through a common interface  |  |  |
